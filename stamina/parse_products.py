#!/usr/bin/env python3
"""
parse_products.py — Build product-lookup dictionaries from one or more
encoded-ID variation CSVs.

Usage:
    python parse_products.py products.csv [extra1.csv extra2.csv ...]

All input files are processed sequentially, sharing the same size and color
dictionaries.  This lets constraints discovered in one file resolve ambiguities
in another.  Output is written alongside the FIRST file.

Output (saved alongside the first input file):
    <stem>_sku.csv       : sku_id    -> product_name   (first file only)
    <stem>_sizes.csv     : size_id   -> size_name      (merged, all files)
    <stem>_colors.csv    : color_id  -> color_name     (merged, all files)
    <stem>_color_map.csv : color_name -> color_id      (merged; ready for
                           migrate_category.py as the colors argument)
    <stem>_errors.csv    : id        -> error_reason   (merged, all files)
...
def main(
    csv_file,
    sku_to_name=None, size_id_to_name=None, color_id_to_name=None,
    size_name_to_id=None, color_name_to_id=None, errors=None,
):
    """
    Parse one encoded-ID CSV and return (sku_to_name, size_id_to_name,
    color_id_to_name, errors).

    If the caller supplies pre-populated dicts (for multi-file processing),
    those are used as the starting state so cross-file constraints apply.
    """
    sku_to_name      = dict(sku_to_name)      if sku_to_name      is not None else {}
    size_id_to_name  = dict(size_id_to_name)  if size_id_to_name  is not None else {}
    color_id_to_name = dict(color_id_to_name) if color_id_to_name is not None else {}
    size_name_to_id  = dict(size_name_to_id)  if size_name_to_id  is not None else {}
    color_name_to_id = dict(color_name_to_id) if color_name_to_id is not None else {}
    errors           = dict(errors)           if errors           is not None else {}
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python parse_products.py <csv_file> [extra1.csv ...]")
        sys.exit(1)

    # ── Shared dictionaries across all input files ────────────────────────────
    sku_to_name      = {}
    size_id_to_name  = {}
    color_id_to_name = {}
    size_name_to_id  = {}
    ...
    # Process primary file first, then extras — all share the same dicts so
    # constraints from one file help resolve ambiguities in another.
    for csv_path in [primary_path] + extra_paths:
        ...
        s, sz, co, er = main(
            ...
            size_name_to_id  = size_name_to_id,
            color_name_to_id = color_name_to_id,
            errors           = errors,
        )
        sku_to_name.update(s)
        size_id_to_name.update(sz)
        color_id_to_name.update(co)
        errors.update(er)
        # Keep reverse dicts in sync after each file
        size_name_to_id  = {v: k for k, v in size_id_to_name.items()}
        color_name_to_id = {v: k for k, v in color_id_to_name.items()}
    ...
    # Inverted color map: color_name -> color_id (for migrate_category.py)
    color_map = {name: id_ for id_, name in sorted(color_id_to_name.items())}
    ...
    csv_outputs = [
        ...
        (color_map,        "color_name","color_id",      stem + "_color_map.csv"),
